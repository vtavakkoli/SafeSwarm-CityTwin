...
def softmax(logits: np.ndarray, temperature: float) -> np.ndarray:
    ...
    return weights / total if np.isfinite(total) and total > 1e-12 else np.ones_like(values) / len(values)
...
class PPOResidualMixin:
    ...
    def __init__(self, *args: Any, model_path: str | Path | None = None, **kwargs: Any) -> None:
        ...
        # External epsilon exploration is removed because its probability was not
        # represented in the stored PPO old_probability. Softmax sampling remains stochastic.
    ...
    def _features(self, env: CityTwinEnvironment, cell: Cell, current: Cell,
                  positions: Dict[int, Cell], agent_id: int) -> dict[str, float]:
        f = super()._features(env, cell, current, positions, agent_id)
        confidence = float(np.clip(1.0 - env.uncertainty_map[cell], 0.0, 1.0))
        # Remove oracle target labels from the trainable actor.
        f["priority"] = float(env.observation_map[cell]) * confidence
        f["target_distance"] = 0.0
        return f

    def _extra_feature_values(self, env: CityTwinEnvironment, cell: Cell,
                              current: Cell) -> tuple[float, float, float]:
        return 0.0, 0.0, 0.0

    @staticmethod
    def _return_progress(env: CityTwinEnvironment, cell: Cell, current: Cell,
                         battery: float) -> float:
        pressure = float(np.clip((0.45 - battery) / 0.45, 0.0, 1.0))
        if pressure <= 0.0:
            return 0.0
        return float(np.clip(
            env.nearest_base_distance(current) - env.nearest_base_distance(cell), -1.0, 1.0
        ) * pressure)

    def _feature_vector(self, env: CityTwinEnvironment, cell: Cell, current: Cell,
                        positions: Dict[int, Cell], agent_id: int) -> np.ndarray:
        ...
        return np.asarray([
            f["priority"], f["uncertainty"], f["pheromone"], f["novelty"], f["spread"],
            f["battery"], f["communication"], -f["congestion"],
    ...
    def _choose_with_residual(self, env: CityTwinEnvironment, agent_id: int, current: Cell,
                              candidates: list[Cell], base_scores: np.ndarray,
                              features: np.ndarray) -> Cell:
        probabilities = softmax(base_scores + features @ self.residual_weights, self.config.temperature)
    ...
    def _propose(self, env: CityTwinEnvironment, agent_id: int, positions: Dict[int, Cell],
                 candidates: list[Cell] | None = None) -> Cell:
        ...
        return self._choose_with_residual(env, agent_id, current, candidates, base_scores, features)
    ...
    def act(self, env: CityTwinEnvironment) -> Dict[int, str]:
        ...
        for agent_id in sorted(env.agents):
            ...
            if state.done:
                ...
                continue
            safe_actions = self.monitor.safe_actions(env, agent_id, planned, count_masked=True)
            if not safe_actions:
                preferred = "STAY"
                _, violations, _ = self.monitor.is_action_safe(env, agent_id, preferred, planned)
                if violations:
                    self.monitor.record_intervention(env, agent_id, preferred, violations)
                action = self.monitor.nearest_safe_action(env, agent_id, planned, preferred)
            ...
            candidate = self._propose(env, agent_id, positions, candidates=candidates)
    ...
    def _parse_sample(sample: tuple[Any, ...]) -> tuple[dict[str, Any], float, float]:
        if len(sample) == 2:
            ...
            return decision, float(advantage), float(decision.get("old_value", 0.0)) + float(advantage)
        if len(sample) == 3:
    ...
    def ppo_update(self, samples: Iterable[tuple[Any, ...]], *, learning_rate: float = 0.03,
                   critic_learning_rate: float | None = None, clip_ratio: float = 0.20,
                   epochs: int = 4, max_grad_norm: float = 2.0) -> dict[str, float]:
        ...
        for _ in range(max(1, epochs)):
            ...
            for (decision, _, target), advantage in zip(parsed, advantages):
                ...
                if use_clipped:
                    ...
                    action_grads.append(float(advantage) * ratio * (features[action] - expected) /
                ...
                if "behavior_base_scores" in decision and hasattr(self, "behavior_bias"):
                    ...
                    bprobs = softmax(bbase + self.behavior_bias[:len(bbase)], self.config.temperature)
                    bnew = float(max(bprobs[baction], 1e-12))
                    bratio = bnew / bold
                    bclip = float(np.clip(bratio, 1.0 - clip_ratio, 1.0 + clip_ratio))
                    b_use_clipped = bclip * float(advantage) < bratio * float(advantage)
                    grad = np.zeros_like(self.behavior_bias)
                    if b_use_clipped:
                        clipped_epoch += 1
                    else:
                        one_hot = np.zeros(len(bbase), dtype=float)
                        one_hot[baction] = 1.0
                        grad[:len(bbase)] = float(advantage) * bratio * (one_hot - bprobs) /
                                           max(0.10, float(self.config.temperature))
                    behavior_grads.append(grad)
                    behavior_kls.append(np.log(bold) - np.log(bnew))

                vf = np.asarray(decision.get("value_features", np.zeros(len(VALUE_FEATURE_NAMES))), dtype=float)
                value = float(vf @ self.critic_weights)
                error = float(target - value)
                critic_grads.append(error * vf)
                value_losses.append(0.5 * error * error)

            grad = np.mean(action_grads, axis=0) - self.residual_l2 * self.residual_weights
            ...
            if behavior_grads and hasattr(self, "behavior_bias"):
                bgrad = np.mean(behavior_grads, axis=0) - self.residual_l2 * self.behavior_bias
                bnorm = float(np.linalg.norm(bgrad))
                if bnorm > max_grad_norm:
                    bgrad *= max_grad_norm / max(bnorm, 1e-12)
                self.behavior_bias += float(learning_rate) * bgrad

            cgrad = np.mean(critic_grads, axis=0) - self.critic_l2 * self.critic_weights
            cnorm = float(np.linalg.norm(cgrad))
            if cnorm > max_grad_norm:
                cgrad *= max_grad_norm / max(cnorm, 1e-12)
            self.critic_weights += critic_lr * cgrad

            policy_loss = float(np.mean(losses))
            value_loss = float(np.mean(value_losses))
            approx_kl = float(np.mean(action_kls))
            behavior_kl = float(np.mean(behavior_kls)) if behavior_kls else 0.0
            clipped += clipped_epoch
            considered += len(parsed) + len(behavior_grads)
        return self._update_diagnostics(policy_loss, value_loss, approx_kl, behavior_kl,
                                        clipped / max(1, considered))

    def _update_diagnostics(self, policy_loss: float, value_loss: float, approx_kl: float,
                            behavior_kl: float, clip_fraction: float) -> dict[str, float]:
        return {
            ...
            "behavior_weight_norm": float(np.linalg.norm(getattr(self, "behavior_bias", np.zeros(1)))),
        }
